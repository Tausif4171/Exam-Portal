...
session_start();
 ...
 $q1="select * from test where createdBy='".$_SESSION['uname']."';";
		...
		if (isset($_GET['id'])) {
			$group_id=$_GET['id'];
			$q2="select * from test where id in (select test_id from assigntest where group_id=$group_id);";
			$assigned_tests=$con->query($q2);
		}
 ?>
<!DOCTYPE html>
<html>
			...
			<div class="row">
				<div class="col-sm-6">
					...
					<div class="form-group ">
					 <select class="form-control" id="test_select" style="text-align: center;">
					   <?php
					...
					<ul>
						<?php 
							while ($r=$assigned_tests->fetch_assoc()) {
								echo "<li>".$r['title']."</li>";
							}
						?>
					</ul>
				...
				<div class="col-sm-6">
					...
					<form action="editGroupX.php" method="POST">
						<div class="form-group col-sm-12">
						    <label >Group name</label>
						    <input type="text" class="form-control" id="email" placeholder="new group name" name="name">
						</div>
						<input type="hidden" class="form-control"  name="id" value=<?php echo "\"".$group_id."\"" ?>>
...
<script src="http://localhost/project008/javascript/editGroup.js">
</script>
</body>
</html>