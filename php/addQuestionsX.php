...
//check whether the last two options are empty make them null 
	if ($opt3=="") {
	$query=	"insert into primaryqb (question,option1,option2,answer,q_bank_id) values('$question','$opt1','$opt2',$answer,$qbid);";
	}
	else if($opt4==""){
		$query=	"insert into primaryqb (question,option1,option2,option3,answer,q_bank_id) values('$question','$opt1','$opt2','$opt3',$answer,$qbid);";	
	}
	else{
		$query=	"insert into primaryqb (question,option1,option2,option3,option4,answer,q_bank_id) values('$question','$opt1','$opt2','$opt3','$opt4',$answer,$qbid);";
	}
	$res=$con->query($query);
	if($res){
		$_SESSION['qadd']="true";
